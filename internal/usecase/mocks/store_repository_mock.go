...
func (m *MockStoreRepository) GetActiveStores(limit, offset int, search string) ([]*domain.Store, int64, error) {
	args := m.Called(limit, offset, search)
	if args.Get(0) == nil {
		return nil, args.Get(1).(int64), args.Error(2)
	}
...
func (m *MockStoreRepository) GetPendingStores(limit, offset int, search string) ([]*domain.Store, int64, error) {
	args := m.Called(limit, offset, search)
	if args.Get(0) == nil {
		return nil, args.Get(1).(int64), args.Error(2)
	}
...
func (m *MockStoreRepository) GetActiveStoreByUserID(userID uint64) (*domain.Store, error) {
	args := m.Called(userID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
