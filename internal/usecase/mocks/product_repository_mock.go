...
func (m *ProductRepositoryMock) GetAllWithFilter(filter *domain.ProductFilter) ([]*domain.Product, int64, error) {
	args := m.Called(filter)
	return args.Get(0).([]*domain.Product), args.Get(1).(int64), args.Error(2)
}

func (m *ProductRepositoryMock) SearchBySlug(slugPattern string, limit, offset int) ([]*domain.Product, int64, error) {
	args := m.Called(slugPattern, limit, offset)
	return args.Get(0).([]*domain.Product), args.Get(1).(int64), args.Error(2)
}

func (m *ProductRepositoryMock) GetByStatus(status string, limit, offset int) ([]*domain.Product, int64, error) {
